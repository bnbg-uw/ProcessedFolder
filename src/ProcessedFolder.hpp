...
namespace processedfolder {
	...
	public:
		...
		std::optional<std::filesystem::path> watershedPolygons(size_t index) const;
		std::optional<std::filesystem::path> watershedPolygons(lapis::rowcol_t row, lapis::rowcol_t col) const;
		lapis::VectorDataset<lapis::MultiPolygon> watershedPolygons(const lapis::Extent& e) const;
